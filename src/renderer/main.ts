...
import { startIdleAnimation, showError as showSpriteError } from "./sprite.js";
...
async function bootstrap(): Promise<void> {
  ...
  enableWindowDrag(petArea);
  setupChat({ history, form, input, send, quit });

  // スプライトシートを Main から URL で取得してアニメ開始。失敗時は代替表示にフォールバック
  try {
    const url = await api.getSpritesheetUrl();
    await startIdleAnimation(canvas, petError, url);
  } catch (err) {
